			...
			<header class="page-header">
                            <?php
                                if( shortcode_exists( 'wp_breadcrumb_taxonomy' ) ) {
                                    do_shortcode( '[wp_breadcrumb_taxonomy page=catalog_book]' );
                                }
			    ...
			    ?>
		            ...
		            <div class="cloud-tags taxonomy-description">
				        ...
				        $element_release = date_i18n( 'j F Y', strtotime( get_book_date_release( $element_id ) ) );
			                $element_title   = ( get_book_title_read( $element_id ) != '' ) ? get_book_title_read( $element_id ) : get_book_title_original( $element_id );

			                $element_sort    = ( get_book_title_sort( $element_id ) != '' ) ? get_book_title_sort( $element_id ) : $element_title;
                                        $element_sort    = strtolower( strtr( $element_sort, $translit ) );
                                        $current_letter  = ( $element_sort ) ? strtolower( $element_sort[0] ) : '';

                                        # Retrieve authors

                                        $element_authors = get_book_author( $element_id );
					            ...
					            echo _x( ' by ', 'book catalog before author elision', 'twentysixteen-child' );
                                                } else {
					            echo _x( ' by ', 'book catalog before author normal form', 'twentysixteen-child' );
                                                }

			                        foreach( $element_authors as $author ){
				                    if( $item_count > 0 ){
                                                        if( $item_count == sizeof( $element_authors )-1 ){
					                    echo _x( ' and ', 'book catalog additional author', 'twentysixteen-child' );
                                                        } else {
                                                            echo ', ';
                                                        }
                                                    }
